...
| **`pi-delegation`** | `skills/pi-delegation/` | **Pi 后台协作**：尽早异步委派边界清楚的编码、只读评审和第二意见，主控可同时推进其他工作；支持分段 `wait`、同记录根目录内的写入互斥、进度流过滤与完整答复留存；禁止嵌套委派，且不会安装给 Pi 自己。 |
...
依赖 `git` 与 bash 4+（macOS 需先 `brew install bash`）。`pi-delegation` 仅支持 Linux，另需 `pi`、`jq`、`setsid`、`flock` 与 GNU `timeout`。
...
- `--with-pi`：拉取子模块并运行 `pi-kit --additive`，安装或升级 Pi 与 pi-kit 管理的 Pi 包，不改动现有设置；没有 Node.js 22.19+ 时会免 root 安装便携版。完成后提示仍缺少的 `jq`、`setsid`、`flock`、`timeout`。
