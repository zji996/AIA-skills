...
主控能同时推进实现、集成或验证时，尽早把这类任务放出去；即使主控自己也能做，低成本、快反馈的独立任务仍值得外包。任务短到委派与验收的开销更高，或下一步决策强依赖尚未整理的上下文时，直接处理。
...
1. **写入互斥**：同一个运行记录根目录内，同一 workdir 同时只允许一个写入任务；并发启动会排队检查，不会同时放行。从不同项目或目录指向同一 workdir 时，统一设置 `PI_DELEGATE_RUNS`，否则彼此的记录不可见。确需并行时各用不同 `--workdir`，或加 `--allow-parallel-writes` 并确保文件不重叠。
